...
function buildLlmsFiles(reportsDir, docsDir, metaData) {
  ...
  const llmsTxt = [
    ...
    'tournament). Win rates are therefore elevated vs. the general player field and',
    'should NOT be treated as absolute competitive benchmarks. Compare detachments',
    'comparatively within this dataset only.',
    '',
    '## How to Interpret',
    '',
    '- Win rate (WR%) = wins ÷ (wins + losses + draws). Sample size (n=) is always shown.',
    '- Tier ratings (S/A/B/C) require n≥3 lists. Detachments below this threshold',
    '  are labelled "Insufficient data" — a single strong finish is not reliable signal.',
    '- "Undefeated" means 0 recorded losses at an event (draws allowed).',
    `- Lists are from ${faction} players at tracked 40K events.`,
    '- AI analysis is generated by Claude with explicit sample-size weighting rules.',
    '  Narrative text uses "Among top-finishing lists in this dataset..." framing.',
  ].join('\n');
